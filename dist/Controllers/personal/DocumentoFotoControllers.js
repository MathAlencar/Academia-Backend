...
class DocumentoControllers {
  ...
  async store(req, res) {
    return upload(req, res, async (err) => {
      ...
      const documento = await _DocumentoFotoPersonal2.default.create({ originalname, filename, personal_id });
  ...
  async update(req, res){
    try{
        ...
        const documento = await _DocumentoFotoPersonal2.default.findByPk(req.params.id);
